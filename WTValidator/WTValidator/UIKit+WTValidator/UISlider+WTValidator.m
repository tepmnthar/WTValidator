...
@end
@implementation UISlider (WTValidator)
...
- (void)setWt_validator:(WTValidator *)wt_validator {
    ...
    [self addTarget:self action:@selector(wt_validate) forControlEvents:UIControlEventValueChanged];
    
}
- (void)wt_validate {
    self.wt_validatorStatus = WTValidatorStatusConverter([self.wt_validator validateInput:[NSNumber numberWithFloat:self.value]]);
